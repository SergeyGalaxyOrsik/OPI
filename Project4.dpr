...
begin
  ...
  while letters_left > 0 do
  begin
    SetLength(mas_size_words, length(mas_size_words) + 1);
   if letters_left <= 7 then
   begin
     mas_size_words[i] := letters_left ;
   end
   else
   begin
    mas_size_words[i] := random(7) + 1;
   end;


   letters_left := letters_left - mas_size_words[i];
...
numberofletters := 20;
ChooseLanguage(Language);
CurrStr := RandomWordInFile(language, numberofletters);
writeln(CurrStr);
level := 1;
f := true;
while f = True do
  ...
  if numberofletters > 190 then f := false;
  readln(InputStr);
  if (InputStr = '13') then F := false;
  if currstr = inputstr then
    begin
    numberofletters := numberofletters - 2;
    CurrStr := RandomWordInFile(language, numberofletters);
    writeln(CurrStr);
    end
    else
